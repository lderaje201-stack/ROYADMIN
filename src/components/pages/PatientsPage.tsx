...
export const PatientsPage: React.FC<PatientsPageProps> = ({
  ...
  return (
    <div id="patients-page" className="p-6 space-y-6">
      {/* Top Controls Bar */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
        <div>
          <h2 className="text-sm font-bold text-slate-900">Registered Clinic Patients</h2>
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors shadow-xs w-full sm:w-auto"
        >
                ...
                filteredPatients.map((p) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4 text-slate-700">
                      <div className="font-semibold">{p.registeredDate}</div>
                      <div className="text-[10px] text-slate-400">Last visit: {p.lastVisit}</div>
                    </td>

                    {/* Doctor */}
                    <td className="py-3.5 px-4">
                      <span className="bg-slate-100 text-slate-800 text-[11px] px-2 py-0.5 rounded font-medium border border-slate-200">
                        {p.assignedDoctor}
                      </span>
                    </td>

                    {/* Reset Password Button per row (Prompt Requirement!) */}
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          id={`reset-pwd-btn-${p.id}`}
                          onClick={() => onOpenResetPasswordModal(p)}
                          className="bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1.5 shadow-2xs"
                        ...
                        <button
                          ...
                          className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-2.5 py-1.5 rounded-lg text-xs font-semibold border border-slate-200 transition-colors"
                        >
                          View Chart
                        </button>
                      </div>
              ...
              <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold px-3 py-1.5 rounded-lg text-xs transition-colors shadow-xs cursor-pointer"
                  >
